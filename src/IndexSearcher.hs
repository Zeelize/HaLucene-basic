...
because its lenght is smaller than size of the first document (we can compute it easily with the equation TF/DF, 
it means number of occcurences).
    ...
    where
        ...
        proccessTermLine tdm (x:xs) = proccessTermLine ntdm xs
            where
                ...
                docId = read $ T.unpack (head split) :: Int
                termFreq = read $ T.unpack (head . tail $ split) :: Int
                ntdm = case M.member docId tdm of
                    True -> M.adjust (+ termFreq) docId tdm 
                    _ -> M.insert docId termFreq tdm
